...
describe("Teste - Login", () => {
    ...
    it("Pagamento - EXT", () => {
        ...
        // clica na opção 10 (dentro do dropdown que abriu)
cy.get('div.ui-select-container[ng-model="currentClass.purchaseInfo.creditCardMaxInstallments"]')
  .filter(':visible')
  .first()
  .find('.ui-select-choices')
  .should('not.have.attr', 'hidden')       // garante que abriu
  .contains('.ui-select-choices-row', /^10$/)
  .click({ force: true });

// valida que ficou selecionado (o texto "Escolher" troca para o valor)
cy.get('div.ui-select-container[ng-model="currentClass.purchaseInfo.creditCardMaxInstallments"]')
  .filter(':visible')
  .first()
  .should('contain', '10');
