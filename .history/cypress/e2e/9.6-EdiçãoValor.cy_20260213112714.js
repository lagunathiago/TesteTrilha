...
describe("Teste - Login", () => {
    ...
    //Pesquisar trilha
        cy.get('.multiselect.ng-pristine > .ng-pristine').type("1401 Teste Lector");

    //Clica em editar trilha
        cy.get(':nth-child(1) > .card-items > .between > .flex > .btn-transparent > .icon-edit').click();

    //clicar em turmas
        cy.get('[trails=""] > .tabs > .ng-scope').click();

    //editar turma
        cy.get('.center > .icon-edit').click();

    //editar valor
        cy.get('.class-price > :nth-child(1) > .icon-checkbox').click({ force: true });
