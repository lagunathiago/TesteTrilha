...
describe("Teste - Login", () => {
    ...
    it("Criando uma trilha paga com aprovação", () => {
        ...
        cy.get('input[placeholder="Informe o nome"]')
            ...
            .focus()
            .clear({ force: true })
            .type('Trilha Automação', { delay: 50 })
        ...
        cy.get('input[ng-model="currentTrail.externalId"]')
            ...
            .focus()
            .clear({ force: true })
            .type('TRILHA_001', { delay: 30 })
            .should('have.value', '202501')

        //seleciona etapas
        cy.get('[trails=""] > .tabs > .tab-active').click();
        cy.get('.tab-content > .row > :nth-child(1) > .btn').click();
        cy.get('.modal-content').should('be.visible');
        cy.get('.modal-content').find('input[placeholder="Pesquisar"]').type('Etapa 1 - Automação');
        cy.get('.modal-content').find('.btn-primary').click();
        cy.get('.tab-content > .row > :nth-child(2) > .btn').click();
        cy.get('.modal-content').should('be.visible');
        cy.get('.modal-content').find('input[placeholder="Pesquisar"]').type('Etapa 2 - Automação');
        cy.get('.modal-content').find('.btn-primary').click();
